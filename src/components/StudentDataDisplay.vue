...
<template>
  <div class="lg:px-36 px-[20px] mb-[40px] xl:mt-[20px] mt-[24px] md:mt-[30px]">
    <div class="flex items-center justify-between">
      <h2 class="text-lg font-medium text-[#1C414F] md:mb-[15px] mb-[12px] ">
    ...
    >
      <div class="lg:flex items-center gap-[4px] flex flex-wrap">
        ...
        >
          <p class="text-[#1C414F] text-sm">{{ selectedData.selectedDirection }}</p>
        </div>
        <div
        ...
        >
          ...
          @click="showFullText" class="overflow-ellipsis text-[#1C414F] text-sm">{{ selectedData.selectedFaculty }}</p>
         <p @click="toggleFullText" class="text-[#1C414F] text-sm" v-else>{{ selectedData.selectedFaculty }}</p>
        </div>
        <div
        ...
        <div
          ...
          class="py-[7px] rounded-[6px] bg-[#F0F3F5] border border-[#D2D2D2] px-[22px]"
        >
          <p class="text-[#1C414F] text-sm ">{{ selectedData.selectedCourse }}</p>
        </div>
        <div
        ...
        <div
          ...
          class="py-[7px] rounded-[6px] bg-[#F0F3F5] border border-[#D2D2D2] px-[22px]"
        >
          <p class="text-[#1C414F] text-sm " >{{ selectedData.selectedSemester }}</p>
        </div>
        <div
          v-if="!selectedData.selectedLanguage"
          class="py-[7px] rounded-[6px] border border-[#D2D2D2] px-[22px]"
        ...
        <div
          ...
          class="py-[7px] rounded-[6px] bg-[#F0F3F5] border border-[#D2D2D2] px-[22px]"
        >
          <p class="text-[#1C414F] text-sm ">{{ selectedData.selectedLanguage }}</p>
        </div>
      </div>
      <button
      v-if="selectedData.selectedLanguage"
        @click="goBack(direction)"
        class="goBackBtn xl:block hidden bg-[#DFF4E0] py-[15px] text-[#2BB231] hover:-translate-x-1 transition pr-[27px] pl-[46px] text-base font-normal rounded-[6px] relative "
      >
        <span  >Изменить</span>
      </button>
    </div>
