<template>
  <div class="lg:px-20 px-[20px] py-[20px] bg-white relative">
    <div class="flex items-center justify-between">
      <router-link to="/" class="lg:w-[216px] w-[188px] mr-[59px] sm:mr-[30px] md:mr-[30px]">
        <img src="../assets/logo.svg" alt="logo" />
      </router-link>

      <button v-if="localStorageDataExists" @click="goToMyDirection()" class="bg-[#2BB231] transition hover:-translate-y-1 rounded-[8px] py-[14px] pl-[40px] pr-[42px] flex items-center gap-[8px] hover:bg-[#7DDD81]">
        <div class="w-[24px] h-[24px]">
          <img src="../assets/icon.svg" alt="icon svg">
        </div>
        <span class="text-white text-base font-normal">{{ $t('Мое направление') }}</span>
      </button>

      <button v-else @click="goToMyDirection2()" class="bg-[#2BB231] transition hover:-translate-y-1 rounded-[8px] py-[14px] pl-[40px] pr-[42px] flex items-center gap-[8px] hover:bg-[#7DDD81]">
        <div class="w-[24px] h-[24px]">
          <img src="../assets/icon2.svg" alt="icon svg">
        </div>
        <span class="text-white text-base font-normal">{{ $t('Выберите  направление') }}</span>
      </button>
...
export default {
  ...
  methods: {
    ...
    goToMyDirection() {
      if(localStorage.getItem("direction") && localStorage.getItem("faculty") && localStorage.getItem("course") && localStorage.getItem("semester") && localStorage.getItem("language")) {
